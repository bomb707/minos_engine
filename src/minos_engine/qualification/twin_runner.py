...
# Accepted TWIN-READY artifact commit (Commit B — where the accepted gate was
# committed). This is a permanent accepted identity, like the accepted gate hash /
# source / tree; the non-mutating check proves HEAD descends from it via Git
# objects. It is NOT the current HEAD and is never derived from commit messages.
ACCEPTED_TWIN_ARTIFACT_COMMIT = "0469b3b8cb326ff32c0412ebec258871c4c39467"
    ...
    *,
    ...
    if require_descends:
        ancestry = verify_commit_ancestry(
            ...
            artifact_commit=ACCEPTED_TWIN_ARTIFACT_COMMIT,
        )
        # The accepted artifact commit must properly descend from the qualified
        # source, and the checked repository HEAD must descend from that accepted
        # artifact commit (supporting execution at the artifact commit OR any later
        # genuine descendant — never HEAD == artifact only).
        checks["commit_b_descends_a"] = ancestry.checks["commit_b_descends_a"]
        checks["artifact_commit_present"] = ancestry.checks["artifact_commit_present"]
        checks["head_descends_artifact"] = ancestry.checks["head_descends_artifact"]
