...
from minos_engine.storage.attempt_workspace import ATTEMPT_DIR_MODE as _ATTEMPT_DIR_MODE
...
from minos_engine.storage.attempt_workspace import (
    reject_symlinked_components as _reject_symlinked_components,
)
...
ATTEMPT_DIR_MODE = _ATTEMPT_DIR_MODE
...
def reject_symlinked_components(path: Path) -> Path:
    """L2-F1's symlink refusal: the shared core under this stage's own typed error."""
    return _reject_symlinked_components(path, error=ExecutionWorkspaceError)


def _create_attempt_dir(work_root: Path, *, job_id: str, attempt_id: str) -> AttemptWorkspace:
    """The L2-F1 execution attempt directory: the shared core under this stage's own name."""
    return create_attempt_workspace(
        work_root, name=f"l2f-{job_id}-{attempt_id}", error=ExecutionWorkspaceError
    )
