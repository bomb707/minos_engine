"""CONTROL-PLANE preparation of the L2-F2 Phase-C execution authority.
...
authority at all. This is the Phase-C counterpart of
:mod:`~minos_engine.storage.l2f2_phase_b_prepare`, and identical in shape: Phase C's plan is
persisted by its own boundary and its jobs are materialized batch by batch as racing permits, so
the ONLY thing left for this module is the one authority row that lets the runner resolve them.

Nothing about that row is chosen. Every value is derived from the completed Phase-B ledger by
:func:`~minos_engine.baseline.phase_c.build_l2f2_phase_c_authority` — the plan, the candidate set,
the counts, the schedule and the protocol — and the function accepts no override for any of them.
A caller supplies an engine.

Two Phase-C specifics are worth stating:

* **There is no Phase-C canary.** The canary is a Phase-A concept: a structurally chosen first job
  proving the chain end-to-end before a screen is expanded. Phase C inherits a proven chain and a
  proven runtime, and inventing one would be fabricating scientific structure to satisfy a column.
  ``0020`` requires the column to be NULL for Phase C exactly as ``0016`` did for Phase B.
* **The logical job count is a CEILING.** 500 is what Phase C may spend, not what it must: racing
  eliminates candidates that can no longer reach the top four, and the authority records the
  budget the plan was built against, not a promise to consume it.
...
__all__ = [
    "PhaseCAuthorityConflictError",
    "PhaseCAuthorityPreparationError",
    "PhaseCAuthorityResult",
...
class PhaseCAuthorityPreparationError(MinosEngineError):
    """The baseline store is not in a state a Phase-C execution authority may be recorded into."""


class PhaseCAuthorityConflictError(PhaseCAuthorityPreparationError):
    """A Phase-C authority already exists for this plan and disagrees with the derived one."""


@dataclass(frozen=True)
class PhaseCAuthorityResult:
...
def _require_baseline_connection(conn: Any) -> None:
    """The active baseline store at the exact revision that admits Phase C. Fail closed."""
    database = conn.execute(text("SELECT current_database()")).scalar_one()
    if database != BASELINE_DATABASE_NAME:
        raise PhaseCAuthorityPreparationError(
    ...
    if revision != BASELINE_REVISION:
        raise PhaseCAuthorityPreparationError(
            ...
            "Phase-C execution authority is representable only from that revision onward"
        )


def _plan_id(conn: Any, authority: PhaseCAuthority) -> str:
    ...
    if plan_id is None:
        raise PhaseCAuthorityPreparationError(
            f"the derived Phase-C plan {authority.plan_hash} is not persisted in this store; "
            "an execution authority over a plan that does not exist authorizes nothing"
        )
    return str(plan_id)


def _verify_existing(row: dict[str, Any], authority: PhaseCAuthority) -> None:
    ...
    expected: dict[str, tuple[Any, Any]] = {
        ...
        "candidate_set_hash": (row["candidate_set_hash"], authority.phase_c_candidate_set_hash),
    ...
    if differing:
        raise PhaseCAuthorityConflictError(
            f"the persisted Phase-C execution authority disagrees with the derived authority on "
            f"{differing}; it is append-only scientific lineage and is never repaired"
        )


def prepare_l2f2_phase_c_execution_authority(engine: Engine) -> PhaseCAuthorityResult:
    """Record THE one Phase-C execution authority for the derived Phase-C plan. Idempotent.
    ...
    with engine.connect() as conn, conn.begin():
        ...
        created = row is None
        if row is None:
            # canary_job_key is deliberately absent from the INSERT: Phase C has no canary, and
            ...
            authority_id = str(
                conn.execute(
                    ...
                    ),
                    {
                        ...
                        "cand": authority.phase_c_candidate_set_hash,
            ...
            if str(existing["plan_id"]) != plan_id:  # pragma: no cover - FK makes this unreachable
                raise PhaseCAuthorityConflictError(
                    f"the persisted Phase-C authority binds plan {existing['plan_id']}, not the "
                    f"derived plan {plan_id}"
                )
            _verify_existing(existing, authority)
            authority_id = str(existing["id"])

    return PhaseCAuthorityResult(
