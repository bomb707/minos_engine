...
_START_SIG = f"{_START_FN}(uuid, text)"
_RELEASE_SIG = f"{_RELEASE_FN}(uuid, text)"
...
def _create_claim_functions() -> None:
    ...
    op.execute(
        f"CREATE OR REPLACE FUNCTION {_START_FN}(p_job_id uuid, p_worker_id text) "
        f"RETURNS TABLE({_RETURN_COLS}) LANGUAGE plpgsql SECURITY DEFINER "
        ...
        "   WHERE j.id = p_job_id AND j.status = 'CLAIMED' "
    ...
    op.execute(
        f"CREATE OR REPLACE FUNCTION {_RELEASE_FN}(p_job_id uuid, p_worker_id text) "
        f"RETURNS TABLE({_RETURN_COLS}) LANGUAGE plpgsql SECURITY DEFINER "
