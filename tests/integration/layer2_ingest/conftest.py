...
def registry_record(art: dict[str, Any]) -> dict[str, Any]:
...
def _seed_registry_and_epoch(url: str, art: dict[str, Any]) -> str:
    """Insert the registered identity + a 1-sample epoch-1 split (owner connection)."""
    rec = registry_record(art)
        ...
        with eng.begin() as c:
            ...
            snap_id = c.execute(
                text(
                    ...
                    " (1, 's', 'v2', :h, :mh, :rsh, :h, 0, 1, 1, 0, 0) RETURNING id"
                ),
...
def l2d_env(pg_base_url: str, l2d_artifacts: dict[str, Any]) -> Iterator[dict[str, Any]]:  # noqa: F811
    ...
    with scratch_database(pg_base_url, "minos_l2d_main") as url:
        ...
        reg_id = _seed_registry_and_epoch(url, l2d_artifacts)
        attestation = attest_input(
            bam_path=l2d_artifacts["bam"],
            bai_path=l2d_artifacts["bai"],
            reference_path=l2d_artifacts["reference"],
            fai_path=l2d_artifacts["fai"],
            registry_record=registry_record(l2d_artifacts),
            registry_snapshot_hash="7" * 64,
        )
        yield {"url": url, "registry_id": reg_id, "attestation": attestation, **l2d_artifacts}
