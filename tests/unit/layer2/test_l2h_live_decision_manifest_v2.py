...
# --------------------------------------------------------------------------- #
def test_the_dispatcher_requires_a_sealed_authority_before_reading_scope(live) -> None:
    ownership = live["ownership"]
    ...
    with pytest.raises(SafeControllerAuthorityError, match="SEALED verified round/profile corpus"):
        safe_decision_manifest_for(request=None, authority=None, ownership=Lookalike())
