...
| Current task | **L2-G — TRAIN CAMPAIGN EVIDENCE PUBLICATION.** OOF records are retained in a `TrustedL2GTrainCampaign` (module-private mint) until published; artifacts are written atomically and hashed from their own bytes; the result binds promotion metrics so an offline verifier re-derives the shortlist rather than trusting it. All protected identities and all ten spec hashes UNCHANGED. **No real campaign has been run** |
