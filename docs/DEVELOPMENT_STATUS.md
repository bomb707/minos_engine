...
| Current task | **L2-H — DECISION-MANIFEST v2 PROOF CLOSURE (source corrective).** Two narrow proof defects in the manifest API, both reproduced. (1) **`owned=` bypassed the ownership proof.** The builders read `owned if owned is not None else ownership.require_owned_request(request)`, so supplying `owned` skipped the proof entirely — and `OwnedRoundProfile` is deliberately public and constructible, immutable but not authoritative. A fabricated member handed in beside a *genuine sealed LIVE authority* produced a v2 manifest carrying real ownership anchors and a real intake identity together with attacker-chosen `profile_id`/`profile_sha256`/`round_id`. `_proven_member` now proves the request unconditionally and accepts `owned` only when it **is** the frozen object the sealed authority holds — object identity, not field equality, since a byte-identical twin is not the corpus member. The parameter stays because byte-locked `storage/decision_persistence.py` passes it; it is a redundancy check now, not a shortcut. (2) **The manifest API never verified the baseline authority.** Only `select_safe_baseline` did, yet the builders are exported and feed `authority.policy`, `baseline_config_hash`, `parameter_space_hash` and `source_commit` straight into a scientific document — a lookalike set `selected_config_hash` to attacker bytes. `_require_manifest_authorities` now demands both sealed capabilities at every public entry, before either is read; a test records every attribute access on a forged authority and asserts none happened. Identity hashers stay pure. All 200 TRAIN v1 manifests still digest to `993632d0…798e09cd`; corpus identity `9cc53b5d…970b03`; LIVE still selects `157d88d1…`. **SOURCE ONLY — still not qualified, not frozen; r0003 pending; service blocked; no gate issued** |
| Previous task | **L2-H — LIVE DECISION-MANIFEST v2 (SOURCE ONLY).** The controller could authenticate a live round but had no scientific contract to record a decision about one: v1 is TRAIN-only and refused live authority, so the live path terminated at the manifest. New `l2h-safe-decision-manifest-v2`, own domain `minos:l2h-safe-decision-manifest:v2`. Four v1 fields describe the frozen research campaign and are **dropped rather than reused** — `dataset_id` (a registered research dataset), `registry_snapshot_hash` (the frozen registry snapshot), `profile_corpus_identity` (the 50-member corpus), `profile_ownership_anchors` (Phase-A/schedule/split/protocol) — and the live authorities are published under their own names: `ownership_scope`, `live_input_set_id` (content-addressed, explicitly not a split allocation or catalog identity), `live_intake_identity`, `live_intake_schema`, `platform_receipt_identity`, `live_profile_ownership_identity`, `live_profile_ownership_anchors`. The internal L2-D-shaped `registry_snapshot_hash` is **not** leaked under that misleading name; the builder instead requires it to equal the published `live_intake_identity`. The shared science is now one implementation used by both schemas, audited field by field. Dispatch is on the SEALED ownership scope after the seal check — never on caller input — and separation is strict both ways: v1(TRAIN) PASS / v1(LIVE) REFUSE, v2(LIVE) PASS / v2(TRAIN) REFUSE. Proven end to end through the real production verifiers (monkeypatched official type resolvers only, **no private mint tokens**): receipt → downloads → intake → real L1 attestation → binding → `VerifiedRoundProfileAuthority(scope=live)` → v2 manifest → exactly the accepted SAFE config `157d88d1…`. All 200 TRAIN v1 manifests byte-identical to entry HEAD; TRAIN corpus identity `9cc53b5d…970b03`. **SOURCE ONLY — not qualified, not frozen. `r0003` is required next: the overlay CHECK still pins v1, and both recording paths fail closed for a live decision. Service still blocked; no gate issued** |
| Previous task | **L2-H — LIVE AUTHORITY SOURCE FINALIZATION.** Two PRE-MINT defects. (1) **TOCTOU on the external client.** Verification checked the exact official type, `demo`, the hotkey and HTTPS, then sealed a wrapper around the *live* `MinerPlatformClient` — which reads `self.demo` (choosing `/v2/round-status` vs `/v2/demo/round-status`), `self.config.base_url` and `self.keypair` **at request time**, with `PlatformConfig` a mutable dataclass whose HTTPS check runs only in `__init__`. So a client verified as live and HTTPS could be flipped to demo and still mint a sealed `ProductionRoundStatusReceipt` from the demo namespace, with `endpoint_path()` still reporting `/v2/round-status`. The wrapper now snapshots the whole binding (base URL, hotkey SS58, round-status path, demo) as engine-owned primitives, and re-checks it against the live object **before and after** every request — re-resolving the official type, requiring the current state to be independently valid, and requiring it to equal the snapshot field for field; an in-flight change discards the payload. The engine's endpoint constant stays as defense in depth, explicitly not as proof. (2) **`ACCEPTED_REFERENCE_IDENTITIES` was mutable** — a dict of writable objects, so which genome the engine accepts as GRCh38 chr20 could be rewritten at runtime without touching source (`typing.Final` enforces nothing). `ReferenceIdentity` is a frozen slotted dataclass, the table a read-only mapping, and `accepted_reference_set_identity()` names the set for audit without entering `LIVE_INTAKE_SCHEMA`. All five contigs byte-identical; TRAIN corpus identity `9cc53b5d…970b03`; v1 manifests unchanged. **Service still blocked; no gate issued** |
| Previous task | **L2-H — VERIFIED CAPABILITY STATE FREEZE.** A capability could be minted correctly, sealed correctly, and then edited. (1) Every sealed capability was mutable afterwards, so a legitimately loaded `VerifiedRoundProfileAuthority` accepted `partition = "live"`, a forged `corpus_identity`, an inserted `_by_round` member and an edited `OwnedRoundProfile` while still answering True to its own seal check — the seal then certified a document that no longer existed. Same for the binding (`owned`, `identity`, `anchors[...]`) and the safe-baseline authority (`baseline_config_hash`, `baseline_uri`, `parameter_space_hash`, `source_commit`, `_policy[...]`). All thirteen sealed capabilities now freeze at the end of their own constructor, nested containers included: read-only views over defensive copies, a deep-frozen policy, a field-frozen `OwnedRoundProfile`, and public accessors that hand back detached plain `dict`/`list`. (2) `_expected_token` published the real mint token on the exported classes, so reading two class attributes turned a fixture chain back into a genuine `VerifiedRoundProfileAuthority(partition="live")` — the previous task's whole split was ornamental. Tokens now live in module-private tables keyed by exact type; the generic `factory`/`factory._expected_token` dispatch is gone, shared validation returns data and mints nothing, and `RoundDownloads`' vacuous `token is not expected` check (both supplied by the same caller) is closed. The trust model is written down rather than overstated: this is not a sandbox against hostile in-process code, and the externally-owned `minos_subnet` objects are snapshotted, not frozen. TRAIN corpus identity still `9cc53b5d…970b03` over 50 rounds; v1 decision manifests byte-identical to entry HEAD. **Service still blocked; no gate issued** |
| Previous task | **L2-H — LIVE OWNERSHIP CAPABILITY-SCOPE CLOSURE.** Two ways authority could be obtained without earning it. (1) One `VerifiedLiveRoundIntake` class, one private token, and a **mutable `scope` string** distinguished production from fixture — while the binding verifier required only `isinstance`. So the offline replay's own intake minted a genuine `VerifiedRoundProfileAuthority(partition="live")`: a test fixture was live authority. Production and fixture are now separate capabilities with separate tokens at every stage — intake, binding and ownership — `scope` is a class attribute of each concrete type, a minted intake is immutable, and the fixture chain ends in `FixtureRoundProfileAuthority`, which no controller entry point accepts. The intake scope is anchored *into* the ownership identity, so the two chains cannot even produce the same identity. (2) `VerifiedLiveProfileBinding`, `VerifiedRoundProfileAuthority` and `VerifiedSafeBaselineAuthority` were still taken on `isinstance`, so a subclass skipping `__init__` and copying the fields reached the decision core — the old generic raw-data mint, through inheritance. All three are sealed and all four controller call sites check exact type + seal; the controller source now contains no `isinstance(authority` or `isinstance(ownership` at all. The seal is runtime state and is excluded from `identity_content()`: the TRAIN corpus identity is still `9cc53b5d28c8a8da34c25095362c09d8cb1fb57533ff0a0b3e1fdf7000970b03` over 50 rounds, and the v1 decision manifest is byte-identical. **Service still blocked; no gate issued** |
| Previous task | **L2-H — LIVE OWNERSHIP SOURCE CLOSURE.** Two narrow defects. (1) The capability wrappers used exact type + seal, but the **raw upstream** checks still used `isinstance`, so a subclass of the real `MinerPlatformClient` or `Miner` overriding `get_round_status` / `_download_bam` could earn a genuine sealed capability. Now `type(x) is official`, with subclass negatives proven by monkeypatching the resolver so they run without the subnet's dependencies. (2) Freshness used `mtime >= marker` — a **fail-open equality** boundary under a coarse filesystem clock. Replaced with a pre/post **inode-state** proof over a path derived from the official `BASE_DIR` + `safe_round_dir_name`; timestamps now decide nothing, so equal-mtime and future-mtime both fail closed. **Service still blocked; no gate issued** |
| Previous task | **L2-H — LIVE OWNERSHIP CAPABILITY + CACHE CLOSURE.** (1) The seal rule was applied only at the transport; downstream still used `isinstance`, so a subclass that skips `__init__` could pass — sharpest case a `ProductionRoundDownloads` subclass carrying a real receipt identity and real binding with attacker hashes. Now every production capability crosses its boundary under `type(x) is C and x._seal is TOKEN`, the `VerifiedOfficialMiner` subclass bypass is closed, and the operational sentinel is private (`receipt_owns_downloads`). (2) The official downloader caches by existence when no `bam_sha256` is published — and `/v2/round-status` does **not** guarantee one (documented only for practice, 'when configured'; read with `.get` by miner and validator) — so bytes fetched under response A could be minted under response B. Closed with a provenance sidecar recording a **digest** of the operational source set; nothing is deleted or re-downloaded and `minos_subnet` is untouched. **Service still blocked; no gate issued** |
| Previous task | **L2-H — LIVE OWNERSHIP AUTHORITY CLOSURE.** Two final defects. (1) `ProductionRoundStatusTransport` was a public subclassing point, so a subclass could mint a real production receipt, and the client was accepted on class/module **names** — a test literally spoofed it. Now the real `utils.platform_client.MinerPlatformClient` and `neurons.miner.Miner` are **imported and checked by type identity**, failing closed with no structural fallback when the subnet package is absent; the transport is `@final`, token-minted, sealed, and verified by exact type. (2) A source-URL string plus a local path never met — nothing proved the file came from that URL. The production API now takes only `(receipt, miner)` and delegates to the maintained `Miner._download_bam`, hashing what it produced. Receipts also carry a per-instance `operational_binding`, because two responses for one round/region/endpoint share a scientific identity. **Service still blocked; no gate issued** |
| Previous task | **L2-H — LIVE OWNERSHIP FINAL SOURCE CORRECTIVE.** Two authority gaps closed. (1) Any transport could mint the one receipt class, and the endpoint check was `startswith('/')` — so a fixture, and `/v2/demo/round-status`, produced a capability production accepted. Now `ProductionRoundStatusReceipt` and `FixtureRoundStatusReceipt` are separate capabilities with separate tokens; production demands a `ProductionRoundStatusTransport` **and** exactly `/v2/round-status`; the subnet client is checked on class, module, HTTPS and non-demo; both scopes share one `parse_round_status`. (2) Download digests proved only that local files were hashed. Now `accept_*_round_downloads` binds receipt → the exact URL that receipt carries → local file → bytes, re-applies the platform's published `bam_sha256`, and records `locally-indexed` for a samtools-built index. The engine does **not** reimplement the miner's downloader. No URL reaches any identity — slot names only. **Service still blocked; no gate issued** |
| Previous task | **L2-H — LIVE OWNERSHIP SECURITY CORRECTIVE.** Three real defects in the first attempt. (1) The intake was self-consistent, not authenticated: a caller could invent a round id, region and four hashes and mint the capability. Now `VerifiedPlatformRoundStatus` exists only because a `RoundStatusTransport` asked the platform, `verify_live_round_intake` has **no content parameter**, and BAM/BAI identities come from `hash_downloaded_inputs` reading real files. (2) `mint_verified_ownership` was a public wrapper around the private token — removed, replaced by `own_verified_live_round(VerifiedLiveProfileBinding)`; the old bypass is executed in a test and refused. (3) The round-id rule accepted **naive** timestamps; the live branch is now upstream's rule transcribed, and verified against the real `minos_subnet` implementation on every case. The replay runs the whole production chain — real BAM, real L1, real `attest_input` — with only the transport faked. **Service still blocked; no gate issued** |
| Previous task | **L2-H — LIVE ROUND/PROFILE OWNERSHIP SOURCE.** `l2h-round-profile-ownership-v2` is TRAIN-only, so no new SN107 round could reach the controller. Traced the real upstream: the platform's `POST /v2/round-status` owns `round_id` and issues it as an **ISO-8601 timestamp**, which the engine's `^[0-9a-f]+$` attestation pattern made inexpressible — widened once, in `intake.contracts.validate_round_identifier`, hex still a strict subset. New `l2h-live-round-intake-v1` (closed field set, re-derived from its own inputs) and `l2h-live-round-profile-ownership-v1` (round-scoped, one member). The `isinstance` capability check is unchanged; a second verifying factory earns the same sealed token. **Decision-manifest v2 IS required** — `profile_ownership_anchors`, `profile_corpus_identity` and `dataset_id` would change meaning — so v1 now refuses a live authority rather than reinterpreting them. Consequently the S3 controller source changed and **SAFE-CONTROLLER-FROZEN does not authorize it**; an `r0003` and a persistence recheck will follow. Operational storage needs a new live registration surface (`ingest_profile` is split-epoch bound) — designed, not built. **Service still blocked; no gate issued** |
| Previous task | **L2-H — PERSISTENCE SECURITY CORRECTIVE.** `r0001` granted `minos_live` raw SELECT on `profiling.bam_profiles` and `catalog.dataset_registry`, and the v1 qualification certified that as least privilege because a SQL recorder showed the code never scanned them. Behaviour is not capability: `SET ROLE minos_live; SELECT count(*) FROM profiling.bam_profiles` returned all 75 identities, TEST included. `r0002_l2h_live_profile_lookup` revokes both and replaces them with a hardened SECURITY DEFINER lookup (`minos_admin`-owned, fixed search_path, no dynamic SQL, two mandatory scalars, ≤1 row, PUBLIC revoked, EXECUTE to `minos_live` alone). `r0001` is not rewritten. The v2 campaign runs on a connection that has **assumed minos_live**, which v1 never did — and which is why v1 could not see that the live role could not read its own schema-version tables. Every enumeration attempt now refused with SQLSTATE 42501; **Service still blocked; no gate issued**; SAFE config row still unprovisioned, and no new LIVE round can obtain a `VerifiedRoundProfileAuthority` — supporting one will require requalifying the frozen controller source |
| Previous task | **L2-H — PRODUCTION DECISION PERSISTENCE.** The live contract requires the decision to be persisted transactionally before the config is returned; the filesystem publication was deferred, never waived. `runtime.decisions` was a strict subset of the specification's own DDL, and completing it at `0027` would have dragged the operational store through twenty-one scientific migrations. So the runtime schema gets its own lineage (`migrations_runtime/`, tracked in `runtime.alembic_version_runtime`) that **requires** the main chain to be exactly `0005_l2e_feature_view` and refuses `0001`/`0020`/`0026` — which is what keeps it off TRAIN and VALIDATION. The main chain is byte-identical, still one head. `config_id` is retired rather than repurposed; `decision_manifest_hash` gets a real preimage. The profile FK is re-aimed from the vestigial `profiling.profiles` (0 rows, never written) to `profiling.bam_profiles` (75 rows, what L1 ingestion actually writes). 196 decisions persisted for real, 30/30 checks; qualification `e88f6cf8…` (file `9bb98d51…`) from `1b67ee2f…`; overlay applied to the operational store, `runtime.decisions` still 0 rows. External acceptance authority `l2h-decision-persistence-acceptance-v1` pins it from a later commit and returns `service_activation_authorised: false`. **Service still blocked; no gate issued**; SAFE config row still unprovisioned, and no new LIVE round can obtain a `VerifiedRoundProfileAuthority` — supporting one will require requalifying the frozen controller source |
| Previous task | **L2-H — SAFE-CONTROLLER-FROZEN FINAL ACCEPTANCE.** The verifier length-checked `engine_git_sha` instead of proving it, so a forged gate could rename its issuer, recompute `gate_hash`, and pass with the science untouched. Issuer provenance is now proved against git (commit exists, tree exact, issuer source stamps the tool version), and capability scope / contextual-model status / publication disposition are bound as domain-separated identities — 19 bindings, up from 16. Gate content changed, so the first issuance `3c6d9b0b…` is superseded before activation and never accepted; accepted gate is `504e701f…`. Acceptance constants live in a **later** module (`…frozen_acceptance`), the repository's non-circular pattern. Service still blocked, persistence still deferred, no migration |
| Previous task | **L2-H — SAFE-CONTROLLER-FROZEN ISSUED (mode-scoped).** Capability is exactly `SAFE_BASELINE`; the gate does **not** authorize BOUNDED / FULL_CONTEXTUAL / REFINEMENT / a model bundle and does **not** satisfy MODELS-QUALIFIED. A dedicated issuer was required: building it from `qualification["checks"]` would let the superseded v2 report mint it, so the only path runs forwards from the accepted v3 bytes (file SHA → canonical → identity → schema → S3 source → verifier PASS → capability) and the 29 checks are **derived** from authenticated observations and independently recomputed authorities. `qualified_source_git_sha` names S3 `7d064fe8…`; `engine_git_sha` names the issuing checkout. Gate hash `3c6d9b0b…`. `CONTROLLER-FROZEN` and MODELS-QUALIFIED remain absent, `select_config` still raises, persistence still deferred |
| Previous task | **L2-H — FINAL AUTHORITY CLOSURE: Phase-A anchor authenticated, seal guard widened, qualification v3.** v2 *recorded* the Phase-A authority hash instead of verifying it, so a coherent rewrite of the authority and the TRAIN schedule together survived the authority layer. The accepted identity `9ad0ba48…` is now **derived** — accepted source constant → BASELINE-QUALIFIED gate hash → pinned commit `9395c116…` → immutable Phase-A blob → domain re-hash — and the working document must equal it before its schedule SHA is believed. The seal guard now patches `builtins.open`/`io.open`/`os.open` and covers the **entire** run; `test_accessed`/`validation_read` are derived from categorised counters and recomputed by the verifier. Publication converges durably. **v3 PASS**, identity `a0e8840d…`, 50 profiles / 200 decisions, all SAFE. v1 and v2 are historical. No gate, no migration, service still blocked |
| Previous task | **L2-H — QUALIFICATION CORRECTIVE: TEST seal closed, TRAIN ownership anchored, publication hardened.** v1's isolation claim was self-refuting — it read and traversed all 75 snapshot members before skipping non-TRAIN, so TEST identities were enumerated, and its skipped counts came *from* those records; v1 is preserved as history and refused by the new verifier. Ownership v2 takes the 50 TRAIN rounds from the frozen TRAIN-only schedule, anchored through pure source (`BASELINE_SELECTED_HASH` → protocol `c548e190…` → Phase-A authority → schedule SHA `694a8993…`) and opens no sealed document; a hard `io.open` guard enforces and *observes* that. Publication now derives the identity and creates no-clobber via `os.link`. Qualification **v2 PASS**, identity `8408630f…`, 50 profiles / 200 decisions, all SAFE. No gate, no migration, service still blocked |
| Previous task | **L2-H — SAFE-CONTROLLER QUALIFICATION EVIDENCE (no gate, service still blocked).** Both documented gaps closed. **#20 ownership:** requests are now proved against the PROFILE-SNAPSHOT-FROZEN-1 membership + artifact inventory + each member's own documents, reusing `validate_admission` unchanged with the registry identity reconstructed from verified attestation bytes; nine fields checked, every mismatch an authority failure. TEST/VALIDATION members are skipped on their partition label before any artifact is opened. **#21 persistence:** explicit deferral — the single-chain alembic topology makes an operational controller migration reachable only through 21 unrelated scientific migrations, so decisions are published content-addressed (staged, fsynced, renamed, read back) and DB persistence stays a prerequisite for activation. Qualification `l2h-safe-controller-qualification-v1` **PASS** over 50 TRAIN profiles × 4 modes = 200 decisions, all SAFE `157d88d1…`; identity `7d305bcd…`. `SAFE-CONTROLLER-FROZEN` registered, **not issued**. A contract defect is recorded: `profile_manifest_hash` has no canonical definition and was replaced in the manifest by the owned `profile_manifest_sha256` |
| Previous task | **L2-H — SAFE-CONTROLLER SOURCE-AUTHORITY CORRECTIVE.** The policy verifier spot-checked fields and required only that the two L2-G freeze identities be non-empty, never checking `accepted_prerequisites`, the reason strings or the schema identities — so a tampered committed policy could verify. Now exactly one document is valid per authority domain: the verifier re-derives the whole policy from the same root and demands nested equality, on every load. Entry gate, policy, baseline, both freezes, prerequisites and git provenance all resolve against one explicit root; verified commit/tree are minted into the capability and no global lookup survives after minting. Policy identity **unchanged** (`638d6348…`) — bytes did not move. 13 runtime tamper cases refused. Service still blocked, no gate, no migration |
| Previous task | **L2-H — SAFE-BASELINE-ONLY CONTROLLER SOURCE AUTHORITY.** L2-G is CLOSED and contextual model capability is NOT QUALIFIED, so `SAFE_BASELINE` is the only authorizable control mode. Policy `l2h-safe-controller-policy-v1` (`638d6348…`) names exactly one allowed mode; BOUNDED / FULL_CONTEXTUAL / REFINEMENT are disabled and a request for one is reduced to SAFE with `SAFE_BASELINE_FORCED`, never executed. Pure core `select_safe_baseline` has one possible config by construction; global authority failures fail closed, round-level degradation emits the verified baseline. Persistence gap reported, no migration. **No gate issued, `select_config` still blocked, MODELS-QUALIFIED still HOLD** |
...
| Previous task | **L2-G v2 — RELATIVE-ADVANTAGE FINALIST SELECTOR (research protocol frozen; nothing qualified).** New protocol, not a v1 continuation. Target is DELTA vs the safe baseline over the four Phase-D finalists; the TRAIN slice is dense (200/200 cells, all ADMITTED). **Feasibility: a perfect four-finalist oracle would gain only 0.0150 mean utility**; the safe baseline is strictly best on 34/50 BAMs. Four candidate specs and an inner-OOF residual-margin switch rule are frozen. **No v2 model has been fitted** |
| Previous task | **L2-G — REAL TRAIN OOF CAMPAIGN v1: COMPLETE and FROZEN.** All ten specs COMPLETE (5/5 folds, 1040/1040 cells, zero training failures); both reference bars set by CONSTANT_SAFE_BASELINE; **TRAIN shortlist EMPTY** — no contextual candidate cleared either bar. Outcome `NO_CONTEXTUAL_MODEL_QUALIFIED_ON_TRAIN`, frozen at `reports/layer2/l2g-train-oof-campaign-freeze-v1.json` (`1c2039de…`). Campaign source `c9618bcd…`. VALIDATION **not read** and not authorized for campaign v1; TEST sealed |
| Next scientific decision | (A) accept SAFE_BASELINE `157d88d1…` and stop contextual-model promotion, or (B) authorize a NEW versioned TRAIN-only L2-G research protocol. No adaptive continuation of the completed v1 campaign |
| MODELS-QUALIFIED | **HOLD_NO_TRAIN_PROMOTABLE_MODEL** — not issued |
| Previous task | **L2-G — PRE-EXECUTION PUBLICATION INTEGRITY.** Exact pre-fit authority SHA, execution-time provenance capture, `TrustedL2GPublishedEvidence` as a capability, one canonical OOF identity domain, readback-verified atomic writes and staged publication. All protected identities and all ten spec hashes UNCHANGED; the pre-fit authority file is untouched. **No real campaign has been run** |
| Previous task | **L2-G — TRAIN CAMPAIGN EVIDENCE PUBLICATION.** OOF records are retained in a `TrustedL2GTrainCampaign` (module-private mint) until published; artifacts are written atomically and hashed from their own bytes; the result binds promotion metrics so an offline verifier re-derives the shortlist rather than trusting it. All protected identities and all ten spec hashes UNCHANGED. **No real campaign has been run** |
| Previous task | **L2-G — REAL TRAIN CAMPAIGN AUTHORITY.** The production entry `run_real_l2g_train_oof_campaign` accepts four operational handles and no scientific object or callback; the injectable core is private. Campaign result v2 now binds the per-spec completeness it claimed, with a verifier that fails closed. All protected identities and all ten spec hashes UNCHANGED. **No real campaign has been run** |
| Previous task | **L2-G — TRAIN OOF EXECUTION SEMANTICS.** Four execution defects closed before the first real fit: CVaR took 12 of 50 instead of `ceil` 13; the safe-baseline reference selected the lexicographically lowest config instead of `157d88d1…`; two references contradicted their own nested-calibration specs; incomplete models were metricised. All ten spec hashes and the four protected identities are UNCHANGED. **No real campaign has been run** |
| Previous task | **L2-G — TRAIN OOF RUNNER SOURCE.** ModelSpec v3 splits the two heads so the tree and MLP candidates no longer misdescribe their admission estimator; the estimator factory, four frozen references, nested calibration, thread enforcement, bundle verifier and five-fold OOF runner exist in source. **No real campaign has been run** |
| Previous task | **L2-G — PRE-FIT AUTHORITY CLOSURE.** The real training dataset is FROZEN: `l2g-training-dataset-v3` / `d031758c…`, 1040 scientific cells derived by the accepted builder from sealed TRAIN evidence (861 admitted / 149 non-admission / 30 execution failure, 0 conflicts). Six candidate and four reference ModelSpec hashes exist. Runtime pinned exactly. **No model has been fitted** |
...
| Controller gate | **`SAFE-CONTROLLER-FROZEN` ISSUED** — PASS, 29 checks, 19 authority bindings, gate hash `504e701fe77b651c919ebc015dc6911bbca880613014058979b18ab408f88add`, file SHA `1bbc1b1b…`, capability `SAFE_BASELINE_ONLY`, accepted by `l2h-safe-controller-frozen-acceptance-v1`. The first issuance (`3c6d9b0b…`) was superseded before activation and never accepted for promotion. `CONTROLLER-FROZEN` reserved for full contextual capability and unissued |
