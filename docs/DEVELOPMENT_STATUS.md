...
| Current task | **L2-G v2 — EXECUTABLE PRE-FIT AUTHORITY (frozen; nothing fitted).** Protocol and spec versioned to v2: exact 4-inner-fold CV, exactly 120 pooled residuals, `numpy.quantile(method="higher")`, strict `>` switch, per-family transforms, equal-BAM weights. Sealed v2 runner, final-bundle procedure and the one-shot VALIDATION rule are all frozen while VALIDATION is still unread. **No v2 model has been fitted** |
