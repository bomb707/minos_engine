...
| Real-GATK canary boundary | **REAL CANARY: PASS.** One GATK execution (`d1f43fa8…`, 71 962 ms, VCF `a91a2c0e…`) and one exact MINOS_SUBNET score (`advanced_score_100 = 61.8836338270872`, `minos_score = 0.618836338270872`, ADMITTED) completed end to end, independently re-verified against the pinned upstream scorer and by evaluation-hash recomputation, with a terminal replay producing no second score. Phase-A jobs 1..194 remain NOT ENQUEUED and NOT EXECUTED; one canary proves the pipeline, not candidate quality. Historical: the environment was readied at `0012` and migrated to `0013`.** |
